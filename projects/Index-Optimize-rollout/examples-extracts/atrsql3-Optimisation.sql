...
/****** Object:  Job [(DBA) - Optimisation]    Script Date: 4/20/2026 3:50:39 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 4/20/2026 3:50:39 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [(DBA) - Indexes]    Script Date: 4/20/2026 3:50:39 PM ******/
...
GO', 
		@database_name=N'DBA', 
		@output_file_name=N'C:\SQLReports\(DBA) - Optimisation - Indexes ($(ESCAPE_SQUOTE(STRTDT))).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [(DBA) - Statistics]    Script Date: 4/20/2026 3:50:39 PM ******/
...
GO', 
		@database_name=N'DBA', 
		@output_file_name=N'C:\SQLReports\(DBA) - Optimisation - Statistics ($(ESCAPE_SQUOTE(STRTDT))).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'(DBA) - Index Optimisation - Evening Run', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
