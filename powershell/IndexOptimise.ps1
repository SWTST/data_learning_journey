$allServers = Get-DbaRegisteredServer -SqlInstance acul021
...
$indexOptimiseJobServers = @()
$indexOptimiseServers = @()
...
/****** Object:  Job [(DBA) - IndexOptimize - USER_DATABASES]    Script Date: 4/1/2026 1:23:06 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 4/1/2026 1:23:06 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(DBA) - IndexOptimize - USER_DATABASES', 
		@enabled=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Index Maintenance]    Script Date: 4/1/2026 1:23:06 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'(DBA) - Index Maintenance', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=127, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'5025647a-1c66-44a4-9d93-3c2e82c3afd6'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  StoredProcedure [dbo].[IndexOptimize]    Script Date: 4/1/2026 2:02:46 PM ******/
SET ANSI_NULLS ON
GO
...
foreach($server in $allServers){
    ...
    try {
        Write-Host("Looking for Index Optimise on $($server.ServerName)") -ForegroundColor Yellow
        #SSA

        $indexOptimiseServers += Invoke-DbaQuery -SqlInstance $server.ServerName -SqlCredential $ssa -Query $spCheck -EnableException 
        continue
    } catch {
        #Windows
        $indexOptimiseServers += Invoke-DbaQuery -SqlInstance $server.ServerName -SqlCredential $windows -Query $spCheck 
    }


    try {
        Write-Host("Looking for Index Optimise Job on $($server.ServerName)") -ForegroundColor Yellow
        #SSA

        $indexOptimiseJobServers += Invoke-DbaQuery -SqlInstance $server.ServerName -SqlCredential $ssa -Query $jobCheck -EnableException 
        continue
    } catch {
        #Windows
        $indexOptimiseJobServers += Invoke-DbaQuery -SqlInstance $server.ServerName -SqlCredential $windows -Query $jobCheck 
    }
}


$allServerNames = $allServers.ServerName | Sort-Object -Unique
$indexOptimiseServerNames = $indexOptimiseServers.ServerName | Sort-Object -Unique
$indexOptimiseJobServerNames = $indexOptimiseJobServers.ServerName | Sort-Object -Unique


$missingSpServers = $allServerNames | Where-Object { $_ -notin $indexOptimiseServerNames }
$missingJobServers = $allServerNames | Where-Object { $_ -notin $indexOptimiseJobServerNames }

$targetServersBoth = $missingSpServers | Where-Object {$_ -eq $missingJobServers}
$targetServersJobOnly = $missingJobServers | Where-Object {$_ -ne $missingSpServers}
$targetServersSPOnly = $missingSpServers | Where-Object {$_ -ne $missingJobServers}

function New-IndexOptimise{
    params(
        [int]$Mode,
        [string]$SqlInstance,
    ...
    if($mode -eq 1){
        Invoke-DBAQuery -SqlInstance $SqlInstance -Query $spBuild -SqlCredential $credentials -EnableException -Verbose
        Invoke-DBAQuery -SqlInstance $SqlInstance -Query $jobBuild -SqlCredential $credentials -EnableException -Verbose
    }
    if($mode -eq 2){
        Invoke-DBAQuery -SqlInstance $SqlInstance -Query $spBuild -SqlCredential $credentials -EnableException -Verbose
    }
...
foreach($server in $targetServersBoth){

    try {
    #ssa
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 1 -SqlInstance $server -credentials $ssa 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    } catch {
    #windows
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 1 -SqlInstance $server -credentials $windows 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    }
}

foreach($server in $targetServersJobOnly){
    try {
    #ssa
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 3 -SqlInstance $server -credentials $ssa 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    } catch {
    #windows
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 3 -SqlInstance $server -credentials $windows 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    }
}

foreach($server in $targetServersSPOnly){
    try {
    #ssa
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 2 -SqlInstance $server -credentials $ssa 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    } catch {
    #windows
    Write-Host("Creating Index Optimise on $server") -ForegroundColor Yellow
    New-IndexOptimise -Mode 2 -SqlInstance $server -credentials $windows 
    Write-Host("Created Index Optimise on $server") -ForegroundColor Cyan
    }
}